...
// Require the classloader
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/MarkupSitemap' . '/ClassLoader.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
...
class MarkupSitemap extends \ProcessWire\WireData implements \ProcessWire\Module
{
    ...
    public function ___install()
    {
        ...
        $applicableMajorMinor = \ProcessWire\ProcessWire::versionMajor === 2 ? '2.8' : '3.0';
        if (version_compare($processWireVersion, "{$applicableMajorMinor}.16") < 0) {
            throw new \ProcessWire\WireException("Requires ProcessWire {$applicableMajorMinor}.16+ to run.");
        }
    }
     ...
     * @return void
     */
    public function deletePageSitemapOptions(\ProcessWire\HookEvent $event)
    {
     ...
     * @return void
     */
    public function processSettingsTab(\ProcessWire\HookEvent $event)
    {
        // Prevent recursion
     ...
     * @param HookEvent $event
     */
    public function render(\ProcessWire\HookEvent $event)
    {
     ...
     * Responds to \ProcessWire\ProcessPageEdit::buildFormSettings hook
     * @param  HookEvent $event
     * @return void
     */
    public function setupSettingsTab(\ProcessWire\HookEvent $event)
    {
